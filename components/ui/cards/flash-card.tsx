...
import {ArrowLeft, ArrowRight, Brain, CheckCircle, RefreshCw, RotateCcw} from "lucide-react"
...
export function FlashCard({ card, onSwipe }: FlashCardProps) {
    ...
    const opacity = useTransform(x, [-250, -150, 0, 150, 250], [0.7, 1, 1, 1, 0.7]) // More gradual opacity change
    ...
    const swipeLeftOpacity = useTransform(x, [-300, -120, -80, 0], [1, 0.8, 0.3, 0])
    const swipeRightOpacity = useTransform(x, [0, 80, 120, 300], [0, 0.3, 0.8, 1])
                    ...
                    <Card className="w-full h-full bg-[#393E46] border-2 border-[#393E46] shadow-2xl">
                        <div className="p-8 h-full flex flex-col justify-between">
                            <div className="flex justify-between items-start mb-6">
                                <Badge variant="secondary" className="bg-[#00ADB5] text-white border-[#00ADB5]">
                                    {card.category}
                                </Badge>
                                <Badge variant="outline" className="border-[#EEEEEE]/30 text-[#EEEEEE]">
                                    {card.difficulty}
                                </Badge>
                            </div>

                            <div className="flex-1 flex items-center justify-center">
                                <p className="text-xl font-medium text-[#EEEEEE] text-center leading-relaxed">{card.front.statement}</p>
                            </div>
                    ...
                    <Card className="w-full h-full bg-[#393E46] border-2 border-[#00ADB5] shadow-2xl">
                        <div className="p-8 h-full flex flex-col justify-between">
                            ...
                            <div className="flex-1 flex flex-col justify-center space-y-4">
                                <p className="text-xl font-medium text-[#EEEEEE] text-center leading-relaxed">{card.back.statement}</p>
                                <div className="bg-[#222831] rounded-lg p-4 border border-[#00ADB5]/20">
                                    <p className="text-[#EEEEEE]/80 text-sm text-center italic">{card.back.explanation}</p>
                                </div>
                            </div>
